...
function main()
    local query =   [[
        SELECT  deviceid,
                MAP_BLOB_JSON_FETCH(fields, 'reportinginterval', 'value') as reportinginterval,
                MAP_BLOB_JSON_FETCH(fields, 'location', 'longitude') as longitude,
                MAP_BLOB_JSON_FETCH(fields, 'location', 'latitude') as latitude,
                MAP_BLOB_JSON_FETCH(fields, 'battery', '') as battery
        FROM    data.event
        WHERE   foreignid = 'e181197f-b9e6-6c4d-ed4b-6149584b9088'
        LIMIT   100;
    ]]
    local source = {
        Protocol = "cassandra",
