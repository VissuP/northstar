...
// ScaleUpApplicationInstances changes the number of instance an application is running
// 		name: 		the id used to identify the application
// 		instances:	the number of instances you wish to change to
//    force: used to force the scale operation in case of blocked deployment
func (r *marathonClient) ScaleUpApplicationInstances(name string, instances int, force bool) (*DeploymentID, error) {
	id := validateID(name)
	app, err := r.Application(id)
	if err != nil {
		return nil, err
	}
	//Scaled instance but still not up and running
	if err == nil && !app.AllTaskRunning() {
		return nil,errors.New("Application tasks are not up and running")
	}
	changes := new(Application)
	changes.ID = id
