...
type Marathon interface {
	...
	// scale a application
	ScaleUpApplicationInstances(name string, instances int, force bool) (*DeploymentID, error)
...
func (r *marathonClient) buildAPIRequest(method, uri string, reader io.Reader) (request *http.Request, member string, err error) {
	...
	if r.config.DCOSToken == "" && r.config.AuthType == "OAUTH" {
		r.config.DCOSToken, err = r.getAuthToken()
		if err != nil {
			r.config.DCOSToken = ""
			return nil, member, err
		}
	} else if  r.config.AuthType != "OAUTH" && r.config.HTTPBasicAuthUser != "" && r.config.HTTPBasicPassword != "" {
...
func (r *marathonClient) getAuthToken() (token string, err error) {
	requestMap := make(map[string]string)
	requestMap["uid"] = r.config.HTTPBasicAuthUser
	requestMap["password"] = r.config.HTTPBasicPassword
	data, err := json.Marshal(requestMap)
	if err != nil {
		r.debugLog.Printf("Error marshaling auth information. %s", err.Error())
		return "", err
	}

	request, err := http.NewRequest("POST", r.config.AuthUrl, bytes.NewBuffer(data))

	if err != nil {
		r.debugLog.Printf("Error creating new request %s", err.Error())
		return "", err
	}
	request.Header.Add("Accept", "application/json")
	request.Header.Add("Content-Type", "application/json")
	...
	if err != nil {
		r.debugLog.Printf("Error processing request %s", err.Error())
		return "", err
	}
	defer resp.Body.Close()
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		r.debugLog.Printf("Error reading response %s", err.Error())
		return "", err
	}
	respMap := make(map[string]string)
	err = json.Unmarshal(body, &respMap)
	if err != nil {
		r.debugLog.Printf("Error unmarshaling auth response %s", err.Error())
		return "", err
	}

	if resp.StatusCode == 200 {
		if token, ok := respMap["token"]; ok {
			return token, nil
		}

	} else {
		if title, ok := respMap["title"]; ok {
			return "", fmt.Errorf("Error: %s", title)
		}
	}
