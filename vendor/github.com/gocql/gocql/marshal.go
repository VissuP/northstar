...
func marshalFloat(info TypeInfo, value interface{}) ([]byte, error) {
	...
	case string:
		...
		if err != nil {
			return nil, marshalErrorf("can not marshal string to float: %s", err)
		}
		return encInt(int32(math.Float32bits(float32(f)))), nil
	}
...
func marshalDouble(info TypeInfo, value interface{}) ([]byte, error) {
	...
	case string:
		...
		if err != nil {
			return nil, marshalErrorf("can not marshal string to double: %s", err)
		}
		return encBigInt(int64(math.Float64bits(f))), nil
	}
	if value == nil {
		return nil, nil
	}
...
func unmarshalTime(info TypeInfo, data []byte, value interface{}) error {
	...
	case *time.Time:
		...
		*v = time.Unix(0, int64(binary.BigEndian.Uint64(data))).In(time.UTC)
		return nil
	}
...
func unmarshalDate(info TypeInfo, data []byte, value interface{}) error {
	...
	case *string:
		...
		*v = fmt.Sprintf("%04d-%02d-%02d", year, int(month), day)
		return nil
	}
	return unmarshalErrorf("can not unmarshal %s into %T", info, value)
}
