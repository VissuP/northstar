...
func (kz *Kazoo) AddPartitions(topic string, numPartitions int, replicaAssignmentStr string, checkBrokerAvailable bool) (err error) {
	...
	if replicaAssignmentStr == "" {
		startIndex := getStartIndex(brokerIDs, existingReplicaListForPartitionZero[0])
		if startIndex < 0 {
			startIndex = 0
		}
	...
	var unmatchedRepFactorList = make(map[int][]int)
	for partition, replicas := range newPartitionReplicaList {
		if len(replicas) != len(existingReplicaListForPartitionZero) {
			unmatchedRepFactorList[partition] = replicas
		}
	}

	if len(unmatchedRepFactorList) != 0 {
		err = fmt.Errorf("The replication factor in manual replication assignment is not equal to the existing replication factor %v", len(existingReplicaListForPartitionZero))
		return
	}
...
func getStartIndex(brokerIDs []int, replica int) int {
	fmt.Printf("brokerIDs: %v, partitionZeroReplicaID: %v\n", brokerIDs, replica)

	if len(brokerIDs) < replica {
		return -1
	}

	return brokerIDs[replica-1]
}
