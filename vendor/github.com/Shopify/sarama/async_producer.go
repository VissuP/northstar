...
import (
	...
	"github.com/Shopify/sarama/vz/breaker"
...
func (p *asyncProducer) newTopicProducer(topic string) chan<- *ProducerMessage {
	...
	tp := &topicProducer{
		...
		topic:       topic,
		input:       input,
		breaker:     breaker.New(3, 1, 10*time.Second, topic, -1),
...
func (p *asyncProducer) newPartitionProducer(topic string, partition int32) chan<- *ProducerMessage {
	...
	pp := &partitionProducer{
		...
		breaker:    breaker.New(3, 1, 10*time.Second, topic, partition),
...
func (p *asyncProducer) newBrokerProducer(broker *Broker) chan<- *ProducerMessage {
	...
	go withRecover(func() {
		for set := range bridge {
			...
			batchTxStartTime := time.Now()
			response, err := broker.Produce(request)
			set.batchTxRxDuration = time.Since(batchTxStartTime)
...
func (bp *brokerProducer) setBatchNwDelay(msgs []*ProducerMessage, batchTxRxDuration time.Duration) {
	for _, msg := range msgs {
		if vzMsgMetadata, ok := msg.Metadata.(*metadata.MessageMetadata); ok {
			vzMsgMetadata.SetBatchNwDelay(batchTxRxDuration)
		} else {
			break
		}
	}
...
func (bp *brokerProducer) handleSuccess(sent *produceSet, response *ProduceResponse) {
	...
	sent.eachPartition(func(topic string, partition int32, msgs []*ProducerMessage) {
		if response == nil {
			...
			bp.setBatchNwDelay(msgs, sent.batchTxRxDuration)
