...
type produceSet struct {
	...
	bufferBytes int
	bufferCount int
	batchTxRxDuration time.Duration
}
...
func (ps *produceSet) wouldOverflow(msg *ProducerMessage) bool {
	switch {
	// Would we overflow segment.index.bytes configuration on the server?
	case ps.parent.conf.Producer.Flush.Bytes > 0 && ps.bufferBytes+msg.byteSize() >= ps.parent.conf.Producer.Flush.Bytes:
		return true
